...
import { listBlog1s, listComments } from '../../../graphql/queries';
...
const CardUI = () => {
  ...
  const handleCardClick = (id: number) => {
    router.push(`/components/BlogCardDetails/?id=${id}`);
  };
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        ...
        // Fetch blogs with comment count
        const blogs = await client.graphql({
          query: listBlog1s,
          variables: { filter: { userId: { eq: userId1 } } },
        });

        const items = blogs?.data?.listBlog1s?.items || [];

        // Assuming each blog has a "comments" relationship that we can query to get the count
        const blogsWithCommentCount = await Promise.all(
          items.map(async (blog) => {
            const comments = await client.graphql({
              query: listComments, // Assuming a query that lists comments for each blog
              variables: { filter: { blogId: { eq: blog.id } } },
            });
            return {
              ...blog,
              commentCount: comments?.data?.listComments?.items?.length || 0,
            };
          })
        );

        // Sort blogs by comment count in descending order
        blogsWithCommentCount.sort((a, b) => b.commentCount - a.commentCount);

        setCardsData(blogsWithCommentCount);
        setFilteredData(blogsWithCommentCount);
      } catch (err) {
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        ...
        if (isEditing) {
          ...
          setCardsData((prev: any) => prev.map((blog: any) => (blog.id === editBlogId ? updatedBlog : blog)));
          setFilteredData((prev: any) => prev.map((blog: any) => (blog.id === editBlogId ? updatedBlog : blog)));
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6'>
        {filteredData.map((card: any) => (
          <div key={card.id} className='bg-white rounded-lg shadow-md p-4'>
            {card.imageKey && (
              <StorageImage imgKey={card.imageKey} alt={card.name} className='w-full h-48 object-cover rounded-lg' />
            )}
            ...
            <p className='text-sm text-gray-500 mb-4'>Comments: {card.commentCount}</p> {/* Display comment count */}
            <div className='flex space-x-2'>
              ...
              <button onClick={() => handleCardClick(card.id)} className='bg-blue-500 text-white py-2 px-4 rounded'>
                View Details
              </button>
            </div>
          </div>
